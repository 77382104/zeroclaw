    ...
    match result {
        Ok(outcome) => {
            ...
            if state.auto_save {
                ...
                let consolidation_provider_ref = turn_provider.clone();
                let consolidation_config = state.config.read().clone();
                zeroclaw_spawn::spawn!(async move {
                    if let Err(_e) = memory_strategy
                        .consolidate_turn(
                            &user_msg,
                            &assistant_resp,
                    ...
                    {
                        let result =
                            match zeroclaw_runtime::agent::agent::build_session_model_provider(
                                &consolidation_config,
                                &consolidation_provider_ref,
                                Some(&model),
                            ) {
                                Ok((provider, _, _)) => {
                        ...
                        if let Err(e) = result {
                            ::zeroclaw_log::record!(
                                ...
                                .with_attrs(::serde_json::json!({
                                    "model_provider": consolidation_provider_ref,
                                    "error": format!("{}", e)
                                })),
